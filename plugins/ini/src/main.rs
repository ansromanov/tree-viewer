//! Bundled INI-family language provider for section folding.
//!
//! Registers the common INI, Systemd, and configuration-file extensions and
//! delegates section detection to the shared core detector.
...
fn register_language_provider(out: &mut impl Write) {
    let msg = serde_json::json!({
        ...
        "params": {
            "extensions": ["ini", "service", "timer", "conf", "properties", "cfg", "desktop"],
            "capabilities": ["fold"]
        }
    });
...
fn handle_open(path: &str, out: &mut impl Write) {
    ...
    let regions = mantis::fold_detectors::section_fold(&content);
