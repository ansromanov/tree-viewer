//! Bundled template language provider for Go-template, Helm, and Jinja2 folds.

use std::io::{self, BufRead, Write};

const EXTENSIONS: &[&str] = &["tpl", "tmpl", "template", "j2", "jinja", "jinja2"];
...
fn register_language_provider(out: &mut impl Write) {
    let msg = serde_json::json!({
        ...
        "params": { "extensions": EXTENSIONS, "capabilities": ["fold"] }
    });
...
fn handle_open(path: &str, out: &mut impl Write) {
    ...
    let regions = mantis::fold_detectors::template_fold(&content);
